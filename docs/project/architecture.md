...
| 2026-09-12 | Criado `packages/frontend/src/services/httpClient.ts` — wrapper genérico de axios (baseURL a partir de `VITE_API_URL`, headers JSON e interceptors) que injeta `Authorization: Bearer <token>` nas rotas protegidas e dispara o evento `AUTH_UNAUTHORIZED_EVENT` quando a API responde `401`; `AuthProvider` o escuta para encerrar a sessão. `authService` deixa de criar sua própria instância de axios. | Preparar a reutilização do cliente HTTP por futuras features sem duplicar a configuração de axios nem a lógica do token |
| 2026-09-13 | Implementado sistema de **storage plugável** (padrão Strategy/Adapter) em `packages/backend/src/storage/` com driver inicial **MinIO** via `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner`. O provider ativo é selecionado por `STORAGE_DRIVER` (factory + `fastify.storage` plugin); trocar de provider (ex.: Cloudinary/Azure/GCS) exige apenas criar um novo adaptador de `IStorageProvider` e registrar um `case` na factory — sem tocar nas regras de negócio. | Necessidade de upload/download de arquivos com capacidade de trocar de provedor de armazenamento sem alterar a lógica de negócio |
| 2026-09-13 | Implementado **UC-010 — Gerenciar Perfil**: rotas `GET /users/me`, `PATCH /users/me` (multipart) e `GET /users/me/photo` (proxy da foto). Nome/e-mail validados pelo schema compartilhado `updateProfileSchema`; unicidade de e-mail com 409. Foto de perfil (PNG/JPG ≤ 400×400, RN-003) validada por parser próprio sem dependências (`utils/image.ts`) e persistida no storage com apenas a chave no banco (`photoKey`). Contrato `IStorageProvider` estendido com `download()` para o proxy — o frontend nunca acessa o store diretamente. `@fastify/multipart` registrado com limites (413). Identificação do usuário sempre via token (`request.user.sub` — RS-001/RS-002). | Atender ao User Flow UC-010 mantendo a foto no serviço de storage configurado e atualizando o perfil com confirmação (RN-004) |
